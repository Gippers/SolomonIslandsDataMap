...
# %% auto 0
__all__ = ['init_load', 'init_init', 'layout', 'initial_load', 'geo_load', 'persist_dd_values', 'persist_shared_values',
           'map_click', 'map_selections', 'update_geography', 'update_measure', 'bar_click', 'update_map',
           'update_bargraph']
...
def initial_load(page_trigger:str, # Page that triggered initial load
    ...
    return val_state['location'], val_state['variable'], val_state['measure']


# %% ../../nbs/03_map_page.ipynb 12
# TODO this defintiely seems hacky, must be a better way
...
def geo_load(page_trigger:str, # Page that triggered initial load
    ...
    """Handles the persistant storage for geo and locks it when required"""
    val_state = json.loads(js)
...
def persist_shared_values(type:str, # Data type to save
                            json_store:dict,
                    ) -> str:
        """Update the data type to persistent on load"""
        store = json.loads(json_store)
