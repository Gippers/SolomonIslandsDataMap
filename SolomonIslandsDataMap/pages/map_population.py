...
# %% auto 0
__all__ = ['init_load', 'init_init', 'layout', 'update_measure_pop', 'update_map_pop']
...
try:
    ...
    from SolomonIslandsDataMap.app_data import mytitle, map_graph, selectedBarGraph, stored_data, dropdown_location  \
        , control_type, dd_var_pop, dd_measure_pop, dd_geo_pop, sidebar_population
...
@callback(
    ...
    allow_duplicate=True,
    prevent_initial_call=True)
def update_map_pop(geo_input:str, # User input from the geography dropdown
    ...
    if button_clicked in [control_type.id, 'initial-load-pop']:
        ...
        for geo in sol_geo.geo_levels:
            tn = np.where(sol_geo.geo_levels == geo)[0][0] # Tracks the trace number
            ar = sol_geo.get_pop(geo_filter = geo, type_filter=data_type, var = variable, measure = measure).values
    ...
    return patched_figure, '## Solomon Islands Population Projections - ' + geo_input
